...
<tr><td class="lefttd">Enter Name:</td><td><input type="text" name="t1" required="required" pattern="[a-zA-Z _]{4,15}" title="please enter only character  between 4 to 15 for donor name" /></td></tr>
<tr><td class="lefttd">Enter Address</td><td><input type="text" name="t_add"  pattern="[a-zA-Z _]{4,15}" title="please enter only character  between 4 to 15 for donor name" /></td></tr>
...
if(isset($_POST["sbmt"])) 
{
    $cn=makeconnection();
    $name = $_REQUEST['t1'];
    $t_add = $_REQUEST['t_add'];
    $r1 = $_REQUEST['r1'];
    $age = $_REQUEST['t2'];
    $mobail = $_REQUEST['t3'];
    $blood_group = $_REQUEST['t4'];
    $email = $_REQUEST['t5'];
    $pwd = $_REQUEST['t6'];

    $res = mysqli_query($cn, "INSERT INTO `user_reg`( `name`, `u_address`, `gender`, `age`, `mobile`, `b_id`, `email`, `pwd`) VALUES('$name','$t_add','$r1','$age','$mobail','$blood_group','$email','$pwd')");
            if ($res) {
...
$s="insert into user_reg(name,u_address,gender,age,mobile,b_id,email,pwd) values('" . $_POST["t1"] ."','" . $_POST["t_add"] ."','" . $_POST["r1"] . "','" . $_POST["t2"] . "','" . $_POST["t3"] . "','" . $_POST["t4"] . "','" . $_POST["t5"] . "','" . $_POST["t6"] .  "')";
